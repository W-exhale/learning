...
- linux查看用户：`cat /etc/passwd`（自己创建的用户在最后）![[Pasted image 20250219105239.png]]
...
![[Pasted image 20250219110409.png]]

## 用户增删改
- 可以在设置里添加
- 创建用户之后会添加一些默认的用户设置![[Pasted image 20250219113340.png]]
- 添加用户：`sudo useradd t1`（t1是用户名）
- 删除用户：`sudo userdel t1`
- 修改用户：`usermod`（能修改`/etc/passwd`中的大部分文件）
	- 可以查看usermod怎么用![[Pasted image 20250219114020.png]]
	- 修改密码：`sudo passwd t1`（必须sudo才能用）![[Pasted image 20250219124757.png]]
...
- 组的目的：共享资源
- 但是linux中不同的发行版不太一样
	- ubuntu会为每一个用户单独创建一个组（与账户名相同），组密码，组ID（GID），属于该组的用户有哪些![[Pasted image 20250219143533.png]]
...
-：是一个文件
l：link文件（符号链接文件）
![[Pasted image 20250219144346.png]]


后面是三个为一组，一共三组：
- rwx：可读，可写，可执行
- rw-：可读，可写，不可执行
![[Pasted image 20250220224020.png]]
- 为什么要分三组？（rwxr-xr-x）
	- 第一组，rwx：创始人的权限
	- 第二组，r-x：下属成员的权限
	- 第三组，r-x：其他组的成员权限

![[Pasted image 20250219145118.png]]

- `chmod`：修改权限的命令（change mode）
- 修改权限时有两种方式（1.用特定字母代表一种，2.用八进制代替）
- 参数：![[Pasted image 20250220225237.png]]
- 符号模式
...
- 八进制：（要记，随意）
	- ![[Pasted image 20250220225333.png]]
	- `chmod 777 file.txt`（将文件改为ugo都可以rwx）
	- SUID位：普通用户在修改文件时可以临时使用root权限（假如说修改密码的时候）![[Pasted image 20250220230633.png]]
